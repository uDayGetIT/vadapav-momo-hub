...
export default function App() {
  ...
  useEffect(() => {
    const chatRefFirebase = db.ref(`rooms/${ROOM_ID}/chat`);
    chatRefFirebase.on('value', snapshot => {
  ...
  const sendMessage = (msgText)=>{
    ...
    db.ref(`rooms/${ROOM_ID}/chat`).set({message:msgObj});
    setText('');
  };
  ...
  return (
    <div className='min-h-screen bg-gray-100 flex flex-col p-2 relative'>
      ...
      <div ref={chatRef} className='flex-1 overflow-y-auto p-2 flex flex-col gap-2'>
        {messages.map(m=>(
